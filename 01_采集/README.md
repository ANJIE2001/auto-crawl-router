...
项目根还有一份 .env  ← ★ 得到大脑的凭证在这，不在上面任何一层
```
...
**唯一要配的是项目根的 `.env`**（放得到大脑的凭证）。没有就照 `.env.example`
复制一份再填。**密钥只放那儿** —— 不进 git，也不会被拖进打包。
...
**搬走**：把文件夹整个拷过去，**别忘了项目根的 `.env`** —— 凭证在里面，
少了它得到大脑跑不起来。只有一种情况要额外处理——如果新位置往上两级不是
