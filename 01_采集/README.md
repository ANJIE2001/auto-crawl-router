...
├── README.md
├── 01_lingzao/              # 灵造（花钱）
│   ├── config.json          # 结构与说明（真值字段留空）
│   ├── collect.py           # 执行逻辑 + 动作定义，自包含
│   └── README.md            # 怎么装、怎么跑
└── 02_getnote/              # 得到大脑（不花钱）
    ├── config.json
    ├── config.local.json    # 老位置，真值已搬走，只剩空壳
    ├── collect.py
    └── README.md

凭证不在这层 —— 存在**你自己电脑**的环境变量 / 注册表里（见下）
```
...
所以没有 `pip install`，也没有虚拟环境。

**凭证不在这层，也不在项目里任何地方** —— 存在你自己的电脑上：

```bash
python 05_技能/set_key.py getnote --client-id cli_xxx --api-key gk_live_xxx
```

默认存成**用户环境变量**（底层写在注册表 `HKCU\Environment`），也可以 `--scope registry`
存进独立注册表项。**项目打包发人时，包里一个 key 都没有** —— 这是刻意的。
...
② 是为了「刚设完还没重启」—— Windows 的环境变量不会自动进已经开着的窗口，直接读注册表就绕过去了。

刚上手先跑这个，它会告诉你缺什么、怎么装：
...
**搬走**：把文件夹整个拷过去就行 —— **凭证不用带**，它在你电脑上（环境变量 / 注册表）。
只有一种情况要额外处理：如果新位置往上两级不是项目根，打开 `config.json` 填一下
`project_root`，告诉它 `02_储存` 在哪。换到**另一台电脑**时，在新机器上跑一次
`python 05_技能/set_key.py` 把 key 存进去。
