...
**存进你自己的电脑 —— 不要写进项目文件**（项目是要打包发人的，写了 key 就被带走了）：

```bash
python 05_技能/set_key.py getnote --client-id cli_xxxxxxxx --api-key gk_live_xxxxxxxx
```

默认存成**用户环境变量**（底层写在注册表 `HKCU\Environment`）。
想藏得更深、不出现在环境变量列表里：加 `--scope registry`，存进
`HKCU\Software\AutoCrawlRouter\getnote`。

> **collect.py 读凭证的顺序**（两个源一样）：
> `① 进程环境变量 → ② 注册表里的用户环境变量 → ③ 独立注册表项 → ④ 项目根 .env（兜底兼容）`
>
> ② 是为了「刚设完还没重启」—— Windows 的环境变量不会自动进已经开着的窗口，
> 直接读注册表就绕过去了。设完立刻就能用，不用重开任何东西。
>
> 同目录的 `config.json` / `config.local.json` **一律留空**，只放结构和说明。
...
去开放平台建一个只勾 `topic.read` + `topic.blogger.read` 的应用，用
`python 05_技能/set_key.py getnote --client-id <新ID> --api-key <新KEY>` 换上，**代码不用动**。
...
拷到哪儿都能跑，只要同目录有 `config.json` —— **凭证不用带，它在你电脑上**。
换到另一台电脑时，在新机器上跑一次 `python 05_技能/set_key.py getnote ...`。
